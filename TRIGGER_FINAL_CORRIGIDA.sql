-- ✅ TRIGGER CORRIGIDA FINAL: fn_insere_taxa_cartao_automatica
-- Esta trigger é disparada por INSERT na tabela CONTA (não antecipacao)
-- Por isso usa NEW.divisao (não NEW.id_divisao)
...
BEGIN
    ...
    -- ✅ CORRIGIDO: Busca usando id_divisao na tabela valor_taxa_cartao
    -- mas pega o valor de NEW.divisao (da tabela conta)
    SELECT valor, descricao 
    INTO v_valor_taxa, v_descricao_taxa
    FROM sind.valor_taxa_cartao 
    WHERE id_divisao = NEW.divisao  -- ✅ CORRIGIDO: tabela conta usa 'divisao', tabela valor_taxa_cartao usa 'id_divisao'
    LIMIT 1;
    
    -- Se não encontrou configuração de taxa, não faz nada
    IF v_valor_taxa IS NULL THEN
        RAISE NOTICE '⚠️ ATENÇÃO: Não há configuração de taxa de cartão para a divisão %', NEW.divisao;
        RETURN NEW;
    END IF;
...
-- 
-- TABELAS E COLUNAS:
-- - sind.conta: usa coluna 'divisao' (NÃO foi renomeada)
-- - sind.antecipacao: usa coluna 'id_divisao' (FOI renomeada)
-- - sind.valor_taxa_cartao: usa coluna 'id_divisao' (FOI renomeada)
--
-- TRIGGER:
-- - É disparada por INSERT na tabela CONTA
-- - NEW.divisao existe (vem da tabela conta)
-- - NEW.id_divisao NÃO existe (tabela conta não tem essa coluna)
--
-- CORREÇÃO APLICADA (linha 43):
-- - WHERE id_divisao = NEW.divisao
-- - Pega NEW.divisao (da tabela conta) e compara com id_divisao (da tabela valor_taxa_cartao)
